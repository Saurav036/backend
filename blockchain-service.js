...
class BlockchainService {
    ...
    async initLedger(org = 'org1') {
        try {
            const { contract, gateway } = await this.fabricUtils.connectToNetwork(org);
            
            console.log(`\n--> Submit Transaction: InitLedger via ${org.toUpperCase()}`);
            await contract.submitTransaction('InitLedger');
            console.log('✅ Result: committed');
    ...
    async getAllAssets(org = 'org1') {
        try {
            const { contract, gateway } = await this.fabricUtils.connectToNetwork(org);
            
            console.log(`\n--> Evaluate Transaction: GetAllAssets via ${org.toUpperCase()}`);
            const result = await contract.evaluateTransaction('GetAllAssets');
            console.log(`✅ Result: ${result.toString()}`);
    ...
    async createAsset(id, color, size, owner, appraisedValue, org = 'org1') {
        try {
            const { contract, gateway } = await this.fabricUtils.connectToNetwork(org);
            
            console.log(`\n--> Submit Transaction: CreateAsset via ${org.toUpperCase()}`);
            await contract.submitTransaction('CreateAsset', id, color, size, owner, appraisedValue);
            console.log('✅ Result: committed');

            gateway.disconnect();
            return { success: true, message: 'Asset created successfully', org };
        } catch (error) {
            console.error(`❌ Failed to submit transaction: ${error}`);
            throw error;
        }
    ...
    // Add other methods with org parameter...
}
